...
import { Vendor, CitizenReport } from '@/data/demoData';
...
const CitizenLeafletMap: React.FC<CitizenLeafletMapProps> = ({ 
  ...
  useEffect(() => {
    ...
    if (vendors.length > 0 || citizenReports.length > 0) {
  ...
  }, [vendors, citizenReports, userRole, onAcceptTask]);
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <h4 className="font-medium">
                          {report.type === 'vendor' ? `Vendor #${report.id}` : `Citizen Report #${report.id}`}
                        </h4>
                        <Badge variant={report.status === 'available' || report.status === 'reported' ? 'default' : 'secondary'}>
                          {report.status}
                        </Badge>
                        <Badge variant="outline">
                          {report.type === 'vendor' ? '📦 Vendor' : '👥 Citizen'}
                        </Badge>
                      </div>
                      <div className="text-sm text-muted-foreground">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          {report.wasteType} {report.type === 'vendor' && `- ${(report as Vendor).quantity}`}
                        </div>
                        {report.type === 'citizen' && (
                          <div className="flex items-center gap-1">
                            <User className="h-3 w-3" />
                            {(report as CitizenReport).citizenName} • {(report as CitizenReport).points} pts
                          </div>
                        )}
                      </div>
                    </div>
